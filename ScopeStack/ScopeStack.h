...
namespace CodeAnalysis
{
	...
	std::unordered_map<size_t, std::string> TreeWalk(element* pItem, std::unordered_map<size_t, std::string> lineNumber, std::string typeVal, bool details = false)
	{
		...
		if (pItem->type_ == "function" || pItem->type_ == "class")
		{
			lineNumber[pItem->startLineCount_] = pItem->type_;
			if (lineNumber.find(pItem->endLineCount_) != lineNumber.end()) {
				if (pItem->type_ == "class")
					lineNumber[pItem->endLineCount_] = "class-end";
				else if (pItem->type_ == "function")
					lineNumber[pItem->endLineCount_] = "function-end";
			}
			else {
				lineNumber[pItem->endLineCount_] = "end";
			}
		}
		...
		{
			lineNumber = TreeWalk(*iter, lineNumber, typeVal);
			++iter;
		}
