 ...
 * --------------------
 * ver 1.5 : 05 Feb 2019
 * - Added functions fileToProcess() and fileDirToProcess()
 * ver 1.4 : 24 Jan 2019
...
namespace FileSystem
{
	...
	public:
		...
		void doFile(const std::string& filename, const std::string& fpath);
		...
		size_t dirCount();
		void showStats();
		const DirExplorerN::files & fileToProcess() const;
		const DirExplorerN::files & fileDirToProcess() const;
	...
	*/
	inline void DirExplorerN::find(const std::string& path)
	{
		...
		for (auto patt : patterns_)
		{
			...
			for (auto f : files)
			{
				doFile(f, fpath);  // show each file in current directory
			}
		}
	...
	inline void DirExplorerN::doFile(const std::string& filename, const std::string& fpath)
	{
		++fileCount_;
		filesToProcess_.push_back(filename);
		filepath_.push_back(fpath);
		std::cout << "\n  --   " << filename;
	}
	...
	inline const DirExplorerN::files&DirExplorerN::fileToProcess() const
	{
		return filesToProcess_;
	}
	//----< return the path of the directory >---------------------

	inline const DirExplorerN::files&DirExplorerN::fileDirToProcess() const
	{
		return filepath_;
	}
}